---
title: 'MCP サーバーフレームワークの比較'
description: 'ローカルおよびリモート展開向けのModel Context Protocol（MCP）サーバーを作成するためのTypeScriptライブラリの包括的な比較。'
---

# <Icon icon="chart-bar" iconType="solid" color="#3B82F6" /> MCPサーバーフレームワークの比較

<Callout type="info">
  <Icon icon="info-circle" iconType="solid" /> Model Context Protocol（MCP）は、アプリケーションが大規模言語モデル（LLM）にコンテキストを提供する方法を標準化するオープンスタンダードです。クライアント・サーバーアーキテクチャに従い、AIモデルが特定のデータや機能に安全にアクセスできるようにします。MCPにより、ホスト（IDEやAIツールなど）はクライアントを介してサーバー（機能を公開する軽量プログラム）と通信し、ローカルまたはリモートのデータソースに安全にアクセスできます。
</Callout>

## <Icon icon="bolt" iconType="solid" /> クイック選択ガイド

このガイドは、主な強みに基づいてフレームワークをすばやく選択するのに役立ちます：

<Cards>
  <Card title="@modelcontextprotocol/sdk" href="#overview-philosophy">
    <Icon icon="code" iconType="solid" /> **選ぶ理由:** 公式の仕様準拠の実装で最大限の制御が必要な場合。基礎的な作業や厳格な準拠が最も重要な場合に最適です。
  </Card>
  <Card title="Cloudflare agents" href="#overview-philosophy">
    <Icon icon="cloud" iconType="solid" /> **選ぶ理由:** 特にCloudflareのエッジネットワーク上でステートフルなAIエージェントを展開する場合。Cloudflareとの統合と管理されたインフラストラクチャの恩恵を受けられます。
  </Card>
  <Card title="fastmcp" href="#overview-philosophy">
    <Icon icon="tachometer-alt" iconType="solid" /> **選ぶ理由:** セッション管理、ストリーミング、本番環境向け機能（認証、ロギングなど）が重要な場合。すぐに使える豊富な機能セットを提供します。
  </Card>
  <Card title="mcp-framework" href="#overview-philosophy">
    <Icon icon="laptop-code" iconType="solid" /> **選ぶ理由:** 規約よりも設定、迅速な開発、CLIツールを好む場合。公式SDKの上に構築され、開発者体験に重点を置いています。
  </Card>
</Cards>

## <Icon icon="list" iconType="solid" /> 機能比較

さまざまな機能で各フレームワークを並べて比較します。

<Tabs items={['概要グリッド', '詳細機能表']}>
  <Tab title="概要">
    | 機能                         | `@modelcontextprotocol/sdk` | `Cloudflare agents`       | `fastmcp`                  | `mcp-framework`            |
    | :--------------------------- | :-------------------------- | :------------------------ | :------------------------- | :------------------------- |
    | **主な目標**                  | コアMCP実装                  | ステートフルなエッジAIエージェント | セッション機能付きMCPサーバー   | 開発者体験重視のMCPフレームワーク |
    ...
    | **開発者体験（DX）**           | 基礎的                      | 高（Cloudflare統合）        | 高（特定機能）               | 高（CLI、規約）             |
...
<Tabs items={['@modelcontextprotocol/sdk', 'Cloudflare agents', 'fastmcp', 'mcp-framework']}>
  ...
  <Tab title="Cloudflare agents">
    - **目標:** Cloudflareのエッジネットワーク上でステートフルなAIエージェントを実現すること。
  ...
  <Tab title="fastmcp">
    - **目標:** 豊富な機能を備えた本番対応のMCPサーバー作成を簡素化すること。
  ...
  <Tab title="mcp-framework">
    - **目標:** 規約とツールを使用してMCPサーバー構築の開発者体験を向上させること。
